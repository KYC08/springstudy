	...
	$.ajax({
		...
		success: (resData) => {  // resData = {"blogList": [], "totalPage": 10}
			...
			$.each(resData.blogList, (i, blog) => {
				let str = '<div class="blog" data-user-no="' + blog.user.userNo + '" date-blog-no="' + blog.blogNo + '">';
...
const fnScrollHandler = () => {
	...
	$(window).on('scroll', (evt) => {
		...
		// 500 밀리초(0.5초) 후에 () => {}가 동작하는 setTimeout 함수
		timerId = setTimeout(() => {
		    ...
		    if( (scrollTop + windowHeight + 50) >= documentHeight ) { // 스크롤과 바닥 사이 길이가 50 px 이하인 경우
		      if(page > totalPage) {
		        return;
		      }
		      page++;
